...
BEGIN
	INSERT INTO public.audit_logs (
		id, actor_id, action, entity, entity_id, summary, result, command_id
	) VALUES (
		'audit-' || substr(md5(random()::text || clock_timestamp()::text), 1, 20),
		auth.uid()::text, _action, _entity, _entity_id, _summary, 'ACCEPTED', _command_id
	);
END;
$$;

REVOKE ALL ON FUNCTION private.append_match_audit(text, text, text, text, text) FROM PUBLIC;
...
BEGIN
	...
	END IF;
	IF (_period <> 'FIRST_HALF' AND current_match.status = 'LIVE')
		 AND (_period <> 'SECOND_HALF' AND current_match.status = 'LIVE') THEN
...
BEGIN
	...
	IF event_type IS NOT NULL THEN
		INSERT INTO public.match_events (
			...
			CASE WHEN _to = 'LIVE' THEN 0 ELSE current_match.clock_seconds END,
	...
	UPDATE public.matches
		 SET status = _to,
				 period = new_period,
				 clock_seconds = CASE WHEN _to = 'LIVE' THEN 0 ELSE clock_seconds END,
				 version = version + 1
	 WHERE id = _match_id AND version = current_match.version
